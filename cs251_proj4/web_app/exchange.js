...
const token_name = 'alwaysCoin';
const token_symbol = 'ABS';

const lp_denominator = 10**9;
...
async function addLiquidity(amountEth, maxSlippagePct) {
  var poolstate = await getPoolState();
  var amountTokens = Math.ceil(poolstate.token_eth_rate * amountEth * (1 + maxSlippagePct / 100));

  var max_exchange_rate = Math.floor(poolstate.eth_token_rate * (1 + maxSlippagePct / 100) * lp_denominator);
  var min_exchange_rate = Math.ceil(poolstate.eth_token_rate * (1 - maxSlippagePct / 100) * lp_denominator);

  // approve the exchange for transfering up to amountTokens tokens
...
/*** SWAP ***/
async function swapTokensForETH(amountTokens, maxSlippagePct) {
  var poolstate = await getPoolState();
  var max_exchange_rate = Math.floor(poolstate.token_eth_rate * (1 + maxSlippagePct / 100) * lp_denominator)
  ;

  await token_contract.connect(provider.getSigner(defaultAccount)).approve(exchange_address, amountTokens);

  await exchange_contract.connect(provider.getSigner(defaultAccount)).swapTokensForETH(amountTokens, max_exchange_rate);
}
...
async function swapETHForTokens(amountEth, maxSlippagePct) {
  ...
  await exchange_contract.connect(provider.getSigner(defaultAccount)).swapETHForTokens(max_exchange_rate, { value: ethers.utils.parseUnits(amountEth.toString(), "wei")});
}
