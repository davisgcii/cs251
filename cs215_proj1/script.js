...
async function getUsers() {
	functionCalls = await getAllFunctionCalls(contractAddress, 'addIOU');
	var users = new Set();

	for (let i = 0; i < functionCalls.length; i++) {
		users.add(functionCalls[i].from);
		if (functionCalls[i].args.length > 0) {
			users.add(functionCalls[i].args[0]);
		}
	}
...
async function getNeighbors(user) {
	return await BlockchainSplitwise.getNeighbors(user);
}
	...
	while (curBlock !== GENESIS) {
		...
		for (var j = 0; j < txns.length; j++) {
			...
			if (txn.to.toLowerCase() === contractAddress.toLowerCase()) {
				...
				// check that the function getting called in this txn is 'addIOU'
				if (func_call && func_call.name === 'addIOU') {
					var timeBlock = await provider.getBlock(curBlock);
					var args = func_call.params.map(function (x) {return x.value});

					if (txn.from.toLowerCase() == user) {
						return timeBlock.timestamp;
					}
					if (args.length > 0 && args[0].toLowerCase == user) {
						return timeBlock.timestamp;
					}
				}
			}
...
async function sanityCheck() {
	...
	console.log('Owed amount is: ', owed);
	...
	console.log('time last active: ', timeLastActive);
